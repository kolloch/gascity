...
// execRaceTimeout is a generous timeout for socket command/response races in
// tests that exercise the control socket across providers.
const execRaceTimeout = 5 * time.Second
...
func requirePrivateFallbackRejected(t *testing.T, p *Provider, name string) {
	...
	checks := []struct {
		...
		{name: "sendSocketCommand", run: func() error {
			return p.sendSocketCommand(name, "ping", execRaceTimeout)
		}},
	}
...
func TestLegacySocketRemainsVisibleWhenPrivateFallbackIsMissing(t *testing.T) {
	...
	for _, want := range []string{"ping", "ping", "interrupt", "stop", "ping"} {
		...
		case <-time.After(execRaceTimeout):
...
// startRecordingControlSocket listens on path and, for each accepted connection,
// records the first line onto the returned channel and replies with response.
// commandBuffer sizes the channel so callers can assert an ordered sequence of
// commands without the listener blocking.
...
// startRejectingControlSocket is a recording socket that always answers with a
// non-"ok" reply, driving the caller down its socket-protocol-error path.
