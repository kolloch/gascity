...
to add cities.`,
	...
	cmd.AddCommand(
		...
		newSupervisorReapCityDoltsCmd(stdout, stderr),
	)
	return cmd
}

// newSupervisorReapCityDoltsCmd builds `gc supervisor reap-city-dolts`, the
// crash-case dolt-cleanup hook wired into the generated systemd unit's
...
// bypasses it and leaves dolt sql-server processes running until the next start
// (pe-t07v, ga-enr). systemd runs ExecStopPost after the main process exits
// regardless of how it died, so this command closes that gap by enumerating the
// registered cities and reaping each one's worktree dolts AND its main managed
// dolt. Reaping the bead-store dolt here is the clean-slate behavior (ga-s5y)
// that retires the hand-installed dolt-cleanup.conf drop-in: it is safe because
// it runs only after the supervisor has stopped.
//
// The legacy "reap-worktree-dolts" name is kept as an alias so a systemd unit
// generated by an earlier `gc` (which still references it in ExecStopPost)
// keeps working until it is regenerated by `gc start`/`gc supervisor install`.
func newSupervisorReapCityDoltsCmd(stdout, stderr io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "reap-city-dolts",
		Aliases: []string{"reap-worktree-dolts"},
		Short:   "Reap a stopped city's dolt processes (worktree + main) for all registered cities (post-shutdown hook)",
		Long: `Reap dolt sql-server processes for every registered city.
...
no-op.`,
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			if runSupervisorReapCityDolts(stdout, stderr) != 0 {
				return errExit
			}
			return nil
		},
...
// runSupervisorReapCityDolts enumerates the registered cities and reaps each
// one's worktree and main managed dolt processes. A missing registry yields an
// empty list (no error), so this is a safe no-op on a host with no cities.
func runSupervisorReapCityDolts(stdout, stderr io.Writer) int {
	entries, err := supervisor.NewRegistry(supervisor.RegistryPath()).List()
	if err != nil {
		fmt.Fprintf(stderr, "gc supervisor reap-city-dolts: reading registry: %v\n", err) //nolint:errcheck
		return 1
	}
	...
	reapCityDoltsForCities(cityPaths, stdout, stderr)
	return 0
}
...
// supervisorDoltReapGrace is the SIGTERM→SIGKILL window applied when reaping
// dolt sql-server processes on shutdown (worktree dolts in-process, and worktree
// + main managed dolts from the post-shutdown reap-city-dolts hook). It mirrors
// the value the operator-installed systemd drop-in used for the main dolt and is
// only ever paid when a city actually has dolts to reap.
var supervisorDoltReapGrace = 2 * time.Second
	...
	for {
		...
		case <-ctx.Done():
			...
			for name, mc := range toStop {
				...
				if reap := reapCityWorktreeDolts(name, nil, nil, supervisorDoltReapGrace, stderr); len(reap.Errors) > 0 {
