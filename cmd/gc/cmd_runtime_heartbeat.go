...
func cmdRuntimeHeartbeat(duration time.Duration, jsonOutput bool, stdout, stderr io.Writer) int {
	...
	// Write held_until through the primary city store directly, mirroring
	// gc runtime request-restart. Upstream routes SESSION-class access through
	// a cliSessionStore helper for a [beads.classes.sessions] relocation; this
	// fork has no such session-class store split, so the primary store is the
	// single source of truth for session beads.
