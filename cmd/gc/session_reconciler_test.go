...
// regression (ga-xxvy / upstream #3994). Heartbeat sets held_until only — no
// sleep_intent and no suspended state — to keep a live session alive through a
// long, silent operation. Before the fix, ComputeAwakeSet's hold suppression
// drove the live session into a "no-wake-reason" drain that force-stopped it
// after defaultDrainTimeout: the exact opposite of the heartbeat's purpose. The
...
// heartbeat crash-recovery gap (ga-xxvy / upstream #3994). A heartbeat hold
// (held_until only, no sleep_intent) defers idle/max-age timers for a LIVE
// session, but must not blind crash recovery: when a heartbeat-held session's
// runtime dies while it still has assigned work, ComputeAwakeSet's hold
// suppression drives ShouldWake=false, so before the fix the respawn arm
...
func TestReconcileSessionBeads_HeartbeatHeldDeadSessionRespawns(t *testing.T) {
	...
	buildDeadHeldSessionWithWork := func(sleepIntent string) (*reconcilerTestEnv, beads.Bead, []beads.Bead) {
		...
		meta := map[string]string{
			...
			// the rapid-crash / churn-productivity windows and reaches the
			// wake/respawn loop as a plain dead-but-desired session.
	...
	t.Run("heartbeat hold respawns dead session with work", func(t *testing.T) {
		...
		heldUntil := before.Metadata["held_until"]

		if woken := reconcileSessionBeadsWithTaskWorkDirSnapshot(t, env, session, work, false); woken != 1 {
		...
		if got := after.Metadata["held_until"]; got != heldUntil {
		...
		if woken := reconcileSessionBeadsWithTaskWorkDirSnapshot(t, env, session, work, false); woken != 0 {
